...
install_packages="postfix postfix-pcre dovecot-imapd dovecot-pop3d dovecot-sieve opendkim opendkim-tools spamassassin spamc net-tools fail2ban bind9-host python3-pandas python3-scikit-learn python3-joblib"
...
smtp inet n - y - - smtpd
  -o content_filter=spambot-filtre
...
spambot-filtre unix -     n       n       -       -       pipe
  flags=Rq user=spambot argv=/etc/spambot/email_filtresi.py \${sender} \${recipient}" >> /etc/postfix/master.cf
...
# Setup spambot
echo "Configuring Spambot files..."
mkdir -p /etc/spambot
cp spam_modeli.joblib /etc/spambot/ 2>/dev/null || true
cp vektorlestirici.joblib /etc/spambot/ 2>/dev/null || true
cp email_filtresi.py /etc/spambot/ 2>/dev/null || true
chmod +x /etc/spambot/email_filtresi.py
useradd -r -s /bin/false spambot 2>/dev/null || true
chown -R spambot:spambot /etc/spambot
